...
// SessionStart writes to user-level ONLY.
...
describe("applyChannelOps soft-warn on missing bundle (issue #299)", () => {
  // When an install-table entry references a bundle whose file is absent,
  // applyChannelOps prints one stderr line `teamagent: skipping channel <ch>
  // — bundle <file> not found` and continues; other channels still install.
  ...
  it("writes a stderr line naming the channel + bundle filename when a user-level bundle is missing", () => {
    ...
    try {
      installHook({
        ...
        userLevel: true,
        homeDir: fakeHome,
        sessionStartEntry: path.join(fakeHome, "does", "not", "exist", "bin-session-start.cjs"),
      });
    } finally {
    ...
    const joined = captured.join("");
    expect(joined).toContain("teamagent: skipping channel SessionStart");
    expect(joined).toContain("bin-session-start.cjs");
  ...
  it("install still succeeds with other channels intact when one bundle is missing", () => {
    ...
    try {
      ...
      const sessionStartList = user.hooks?.SessionStart ?? [];
      expect(sessionStartList).toHaveLength(0);
    } finally {
      fs.rmSync(fakeHome, { recursive: true, force: true });
    }
  });
