...
// SessionStart is user-level only (see channelOps and
...
function applyChannelOps(opts: {
  ...
  for (const def of ALL_CHANNELS) {
    ...
    if (!bundlePath || !fs.existsSync(bundlePath)) {
      // Issue #299: previously a silent `continue` — the missing bundle was
      // dropped without trace. Now we emit one stderr line so the user (and
